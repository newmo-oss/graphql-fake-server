...
class RequestQueue {
    ...
    #running = 0;
    #maxConcurrent = 10; // Reduced default for better stability
                ...
                try {
                    ...
                    if (this.#requestDelay > 0) {
                        ...
                        if (timeSinceLastRequest < this.#requestDelay) {
                            await new Promise((r) =>
                                setTimeout(r, this.#requestDelay - timeSinceLastRequest),
                            );
                        }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
            ...
            if (attempt < maxAttempts - 1) {
                ...
                const baseDelay = Math.min(initialDelay * backoffFactor ** attempt, maxDelay);
...
export function createFakeClient(options: CreateFakeClientOptions) {
    ...
    return {
        async registerListDestinationCandidatesQueryResponse(
            sequenceId: string,
            queryResponse: ListDestinationCandidatesQuery,
            sequenceOptions?: FakeClientRegisterSequenceOptions<ListDestinationCandidatesQueryVariables>,
        ): Promise<{ ok: true }> {
            const requestCondition = sequenceOptions?.requestCondition ?? { type: "always" };
            const response = await requestQueue.add(() =>
                fetchWithRetry(options.fakeServerEndpoint, {
                    method: "POST",
                    headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `Failed to register fake response: ${response.status} ${response.statusText}${errorResult.errors ? ` - ${JSON.stringify(errorResult.errors)}` : ""}`,
                );
            }
            return result as { ok: true };
        },
        async registerListDestinationCandidatesQueryErrorResponse(
            sequenceId: string,
            {
                errors,
                responseStatusCode,
            ...
            if (!response.ok) {
                const errorResult = result as { errors?: string[] };
                throw new Error(
                    `Failed to register fake error response: ${response.status} ${response.statusText}${errorResult.errors ? ` - ${JSON.stringify(errorResult.errors)}` : ""}`,
                );
            }
            return result as { ok: true };
        },
        async calledListDestinationCandidatesQuery(sequenceId: string): Promise<{
            ...
            const response = await requestQueue.add(() =>
                fetchWithRetry(`${options.fakeServerEndpoint}/called`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "sequence-id": sequenceId,
            ...
            if (!response.ok) {
                const errorResult = result as { errors?: string[] };
                throw new Error(
                    `Failed to get called data: ${response.status} ${response.statusText}${errorResult.errors ? ` - ${JSON.stringify(errorResult.errors)}` : ""}`,
                );
            }
        ...
        async registerListRideHistoriesQueryResponse(
            sequenceId: string,
            queryResponse: ListRideHistoriesQuery,
            sequenceOptions?: FakeClientRegisterSequenceOptions<ListRideHistoriesQueryVariables>,
        ): Promise<{ ok: true }> {
            const requestCondition = sequenceOptions?.requestCondition ?? { type: "always" };
            const response = await requestQueue.add(() =>
                fetchWithRetry(options.fakeServerEndpoint, {
        ...
        async registerListRideHistoriesQueryErrorResponse(
            sequenceId: string,
            {
                errors,
                responseStatusCode,
        ...
        async registerCreateUrlRideHistoryMutationResponse(
            sequenceId: string,
            mutationResponse: CreateUrlRideHistoryMutation,
            sequenceOptions?: FakeClientRegisterSequenceOptions<CreateUrlRideHistoryMutationVariables>,
        ): Promise<{ ok: true }> {
            const requestCondition = sequenceOptions?.requestCondition ?? { type: "always" };
            const response = await requestQueue.add(() =>
                fetchWithRetry(options.fakeServerEndpoint, {
        ...
        async registerCreateUrlRideHistoryMutationErrorResponse(
            sequenceId: string,
            {
                errors,
                responseStatusCode,
        ...
        async registerCreateFooUrlRideHistoryMutationResponse(
            sequenceId: string,
            mutationResponse: CreateFooUrlRideHistoryMutation,
            sequenceOptions?: FakeClientRegisterSequenceOptions<CreateFooUrlRideHistoryMutationVariables>,
        ): Promise<{ ok: true }> {
            const requestCondition = sequenceOptions?.requestCondition ?? { type: "always" };
            const response = await requestQueue.add(() =>
                fetchWithRetry(options.fakeServerEndpoint, {
        ...
        async registerCreateFooUrlRideHistoryMutationErrorResponse(
            sequenceId: string,
            {
                errors,
                responseStatusCode,
